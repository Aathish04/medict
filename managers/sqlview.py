...
class SQLManager(object):
    ...
    def write_database(self, columnNames, rows):
        ...
        for row in rows:
            row[sympIndex] = (
                "null" if row[sympIndex] == "UNKNOWN" else json.dumps([row[sympIndex]])
            )
            row[timesIndex] = (
                "null"
            ...
            for i in range(len(row)):
                if (
                    row[i] == "UNKNOWN"
                ):  # Make it to null in SQL instead of saving it as is as it cause errors.
                    row[i] = "null"
    ...
    def update_row(self, patientid, fields_to_change, updated_data):
        ...
        if len(fields_to_change) == len(updated_data):
            ...
            raise ValueError(
                "Looks like fields_to_change and updated_data doesn't have equal values."
            )
